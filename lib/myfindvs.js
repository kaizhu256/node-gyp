...
VisualStudioFinder.prototype = {
  ...
  function findVisualStudio (that) {
    that.configVersionYear = null
    that.configPath = null
    if (that.configMsvsVersion) {
      that.addLog('msvs_version was set from command line or npm config')
      if (that.configMsvsVersion.match(/^\d{4}$/)) {
        that.configVersionYear = parseInt(that.configMsvsVersion, 10)
        that.addLog(
          `- looking for Visual Studio version ${that.configVersionYear}`)
      } else {
        that.configPath = path.resolve(that.configMsvsVersion)
        that.addLog(
          `- looking for Visual Studio installed in "${that.configPath}"`)
      }
    } else {
      that.addLog('msvs_version not set from command line or npm config')
    }
    ...
    that.findVisualStudio2017OrNewer((info) => {
      ...
      that.findVisualStudio2015((info) => {
        if (info) {
          return that.succeed(info)
        }
        that.findVisualStudio2013((info) => {
          if (info) {
            return that.succeed(info)
          }
          that.fail()
...
findVisualStudio(new VisualStudioFinder({ major: 1, minor: 0, patch: 0 }))
